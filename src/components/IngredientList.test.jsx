...
describe('IngredientList', () => {
  it('shows an empty state and no toolbar for zero foods', () => {
    ...
    expect(screen.queryByLabelText(/Ingredient list toolbar/i)).not.toBeInTheDocument();
  });
  ...
  it('defaults one food to expanded with nutrient chips visible', () => {
    ...
    expect(screen.getByRole('button', { name: /Collapse all ingredients/i })).toBeInTheDocument();
  });

  it('defaults many foods to collapsed, then expands and collapses all rows from the lightweight list header', async () => {
    ...
    await user.click(screen.getByRole('button', { name: /Expand all ingredients/i }));
    expect(screen.getAllByText(/Energy: 100 kcal/i)).toHaveLength(12);

    await user.click(screen.getByRole('button', { name: /Collapse all ingredients/i }));
    expect(screen.queryAllByText(/Energy: 100 kcal/i)).toHaveLength(0);
  });
